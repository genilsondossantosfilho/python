...
class Lista:
    def __init__(self):
        self.ultimo = Elemento.Elemento("valorFixoAleatorio",None) 'aponta pra none' 'e Elemento.Elemento é pra pegar importar do modulo/arquivo Elemento o metodo/classe Elemento'
         
          '''criei o elemento final'''
         
        self.primeiro = self.ultimo 'aponta'
       
         '''o primeiro elemento sempre sera o elemento ultimo'''
    ...
    def imprimeLista(self):
        ...
        if aux is not None:
            ...
            print("--- Lista Vazia So tem sentinela ---")
    ...
    def addElementoNoFinal(self, valorQualquer):
        if self.primeiro is None: 'lista vazia self.primeiro == self.ultimo == None'
            novo = self.criarNovoElemento(valorQualquer)
            self.primeiro = novo
            novo.proximo = self.ultimo
            
        else: 'se a lista n estiver vazia, ou seja, self.primeiro != self.ultimo'
        
            aux = self.primeiro 'rodar a lista toda ate encontrar o sentinela'
            
            while aux.proximo is not self.ultimo:
                aux = aux.proximo  'vai chegar no penultimo, anterior ao self.ultimo'
